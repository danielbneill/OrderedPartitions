...
\end{prop}
\begin{proof}
Let $P ^* = (x^*, y^*) \in \mathcal{E}$. Then there is an affine $a(x,y0 = Ax + By + C$ satisfying $a(x^*, y^*) = 0$, $a(x,y) < 0$ for all $(x,y) \in \mathcal{C} \Bigr\rvert_{\{ p^* \}}$.  We have
\begin{align} \label{}
Ax^* + By^* = \min_{(x,y) \in \mathcal{C}} Ax + By = \min_{\pi \subseteq N} \sum_{i \in \pi} (Ax_i + By_i).
\end{align}
The set $S^* = \{ i \in N: Ax_i + By_i \leq 0 \}$ clearly minimzes \ref{eq3}. We will show that $S^*, N\backslash_S^*$ are both consecutive. \\
